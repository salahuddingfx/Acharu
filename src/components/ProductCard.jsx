...
const ProductCard = ({ product }) => {
  ...
  return (
    <motion.div 
      ...
      viewport={{ once: true }}
      className="group relative bg-white rounded-2xl shadow-sm hover:shadow-xl hover:shadow-maroon/5 border border-slate-100 overflow-hidden transition-all duration-500 flex flex-col h-full"
    >
        ...
        <div className="flex-grow">
          <div className="flex items-center gap-2 mb-0.5">
            <Link 
              to={`/product/${product.slug || product.id}`}
              className={clsx(
                "text-[13px] font-bold text-slate-800 hover:text-maroon transition-colors line-clamp-1 leading-tight",
            ...
            {product.weight && (
              <span className="shrink-0 px-1.5 py-0.5 bg-slate-50 text-slate-400 text-[9px] font-black rounded-md border border-slate-100">
                {product.weight}
              </span>
            )}
          </div>
            ...
            {product.stock > 0 && (
              <>
                <button 
                  ...
                  className="w-7 h-7 rounded-md bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-maroon/10 hover:text-maroon transition-all"
                  title="Add to Cart"
                >
                ...
                <button 
                  ...
                  className="w-7 h-7 rounded-md bg-maroon text-white flex items-center justify-center shadow-lg shadow-maroon/10 hover:-translate-y-0.5 transition-all"
                  title="Order Now"
                >
                  <ShoppingBag size={14} />
                </button>
