...
const OrderTracking = () => {
  ...
  const performSearch = async (id, isPoll = false) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // If it's a poll, update the selected order
        if (isPoll && selectedOrder) {
          ...
          // If multiple found and none selected yet, stay in selection mode
  ...
  return (
    <div className="bg-[#FFF9F5] min-h-screen pb-20 pt-24 relative overflow-hidden">
      <Helmet>
        <title>Track Order | Acharu</title>
      </Helmet>
      ...
      <motion.div 
        ...
        className="absolute top-0 right-0 w-[600px] h-[600px] bg-maroon/5 rounded-full blur-[120px] -z-0"
      />
      <motion.div 
        animate={{ rotate: -360, x: [0, -30, 0], y: [0, 40, 0] }}
        transition={{ duration: 30, repeat: Infinity, ease: "linear" }}
        className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-orange-100/30 rounded-full blur-[100px] -z-0"
      />
      ...
      <div className="container-custom relative z-10">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center mb-8">
            ...
            >
              Where is my <span className="text-maroon italic">Product?</span>
            </motion.h1>
            <motion.p 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-3">
              <div className="relative flex-grow">
                ...
                <input 
                  ...
                  className="w-full pl-16 pr-6 py-5 bg-slate-50 border-transparent rounded-[30px] focus:bg-white focus:ring-4 focus:ring-maroon/5 focus:border-maroon/20 outline-none transition-all text-sm font-bold text-slate-800"
                />
              </div>
              <button 
                type="submit" 
                disabled={isSearching}
                className="bg-maroon text-white px-10 py-5 rounded-[30px] font-black uppercase tracking-widest text-[11px] hover:bg-slate-900 transition-all shadow-xl shadow-maroon/20 disabled:opacity-70 active:scale-95 flex items-center justify-center gap-2"
              >
                {isSearching ? (
              ...
              >
                ...
                {orders.map((o) => (
                  <button 
                    ...
                    className="bg-white p-6 rounded-[30px] border border-slate-100 shadow-sm hover:shadow-md hover:border-maroon/20 transition-all text-left flex justify-between items-center group"
                  >
                    <div>
                      <p className="text-[10px] font-black uppercase text-maroon mb-1">#{o.tracking_id}</p>
                    ...
                    <div className={clsx(
                      ...
                      o.status === 'delivered' ? "bg-emerald-50 text-emerald-600" : "bg-maroon/5 text-maroon"
                    )}>
              ...
              >
                ...
                {orders.length > 1 && (
                  <button 
                    ...
                    className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-maroon transition-colors"
                  >
                ...
                <div className="bg-white rounded-[50px] p-8 md:p-16 shadow-[0_30px_100px_-20px_rgba(0,0,0,0.06)] border border-slate-100 relative overflow-hidden">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-10 mb-10 relative z-10">
                    <div>
                      <div className="flex items-center gap-3 mb-4">
                        <div className={clsx(
                          ...
                          selectedOrder.status === 'delivered' ? "bg-emerald-50 text-emerald-600" : "bg-maroon/5 text-maroon"
                        )}>
                    ...
                    <div className="p-6 bg-slate-50 rounded-[30px] border border-slate-100">
                      ...
                      <h3 className="text-xl font-black text-maroon tracking-wider">#{selectedOrder.tracking_id}</h3>
                    </div>
                  </div>
                        ...
                        return (
                          <div key={step} className="flex flex-col items-center relative z-10 group">
                            <div className={clsx(
                              ...
                              isCompleted ? "bg-maroon text-white scale-110" : "bg-white text-slate-300 border border-slate-100 scale-90"
                            )}>
                              ...
                              {isActive && (
                                <motion.div 
                                  ...
                                  className="absolute -inset-2 bg-maroon/20 blur-xl rounded-full -z-10"
                                />
                              )}
                            </div>
                            <div className="text-center mt-4 md:mt-6 px-1">
                              ...
                              {isActive && <p className="text-[8px] md:text-[9px] text-maroon font-bold uppercase tracking-tighter mt-1 animate-pulse">In Progress</p>}
                            </div>
                          </div>
                        );
                      })}
                              ...
                              <div className="flex items-center gap-4 md:gap-6">
                                ...
                                <div className="flex flex-col">
                                  ...
                                  <div className="flex items-center gap-3 mt-1">
                                    <span className="text-[10px] font-black text-maroon uppercase bg-maroon/5 px-2 py-0.5 rounded">Qty: {item.quantity}</span>
                      ...
                      <div className="mt-12 pt-8 border-t border-slate-100 grid grid-cols-1 md:grid-cols-2 gap-10">
                        <div className="space-y-4">
                           ...
                           <div className="flex justify-between items-center pt-4 border-t-2 border-dashed border-slate-100">
                             ...
                             <span className="text-3xl font-black text-maroon">{formatPrice(selectedOrder.total_amount)}</span>
                           </div>
                        </div>
              ...
              !isSearching && orders.length === 0 && (
                ...
                >
                  <div className="relative inline-block mb-8">
                     <div className="absolute inset-0 bg-maroon/5 blur-3xl rounded-full" />
...
const ProductReviewSection = ({ item, order }) => {
  ...
  const [media, setMedia] = useState([]); // { file, type, preview }
    ...
    <div className="mt-6 md:ml-20">
      {!showForm ? (
        <button 
          ...
          className="px-4 py-2 bg-maroon/5 border border-maroon/10 rounded-full text-[10px] font-black uppercase tracking-widest text-maroon hover:bg-maroon hover:text-white transition-all flex items-center gap-2 shadow-sm active:scale-95"
        >
        ...
        >
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => setShowForm(false)} className="text-[10px] font-black text-slate-400 hover:text-maroon">CANCEL</button>
          </div>
          ...
          <div className="flex gap-1">
            {[1, 2, 3, 4, 5].map((star) => (
              <div key={star} className="relative flex items-center">
                ...
                <div className="flex">
                  {rating >= star ? (
                    <Star size={24} fill="currentColor" className="text-maroon" />
                  ) : rating >= star - 0.5 ? (
                    <StarHalf size={24} fill="currentColor" className="text-maroon" />
                  ) : (
          ...
          <textarea 
            ...
            className="w-full bg-white border border-slate-100 rounded-2xl p-4 text-sm focus:outline-none focus:ring-2 focus:ring-maroon/10 focus:border-maroon/20 transition-all resize-none"
            rows="3"
          />
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <label className="w-16 h-16 rounded-xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center text-slate-400 hover:border-maroon/30 hover:text-maroon transition-all cursor-pointer">
              <ImageIcon size={16} />
          ...
          <button 
            ...
            className="w-full bg-slate-900 text-white py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-maroon transition-all disabled:opacity-50 flex items-center justify-center gap-2"
          >
