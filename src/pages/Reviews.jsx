    ...
    <div className="bg-cream min-h-screen pb-20">
      {loading ? (
        ...
        <>
          ...
          <section className="relative bg-maroon overflow-hidden pt-16 pb-24">
            ...
            <div className="container-custom relative z-20">
              <div className="text-center mb-12">
                <span className="inline-block px-4 py-1.5 rounded-full bg-white/10 border border-white/20 text-[10px] font-black uppercase tracking-[0.3em] text-cream/90 mb-6">
                  Customer Love
                </span>
                <h1 className="text-4xl md:text-6xl font-display font-black text-white leading-tight tracking-tighter">
                  What Our Community Says
                </h1>
                <p className="text-cream/70 text-lg mt-4 max-w-2xl mx-auto font-medium">
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between pt-6 border-t border-white/10">
                          <div className="flex items-center gap-4">
                            <div className="w-12 h-12 rounded-full bg-cream/20 flex items-center justify-center text-cream font-black text-lg">
                            ...
                            <div>
                              ...
                              {featured[featuredIdx]?.product && (
                                <p className="text-xs text-cream/60">{featured[featuredIdx]?.product?.name}</p>
                              )}
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="flex items-center gap-2">
                      {featured.map((_, i) => (
                        <button
                          ...
                          className={`w-2 h-2 rounded-full transition-all duration-500 ${i === featuredIdx ? 'bg-cream w-6' : 'bg-white/20 hover:bg-white/40'}`}
                        />
                      ))}
                    </div>
              ...
              {reviews.map((review, idx) => (
                ...
                >
                  ...
                  <div className="pt-4 border-t border-slate-100 mt-auto">
                    ...
                    {review.media && review.media.length > 0 && (
                      <div className="flex flex-wrap gap-2 mt-4">
                        {review.media.map((m, idx) => (
                          <div key={idx} className="relative group w-16 h-16 rounded-xl overflow-hidden border border-slate-100 shadow-sm cursor-pointer hover:border-maroon/30 transition-all">
                            {m.type === 'image' ? (
                              <img 
                                src={m.file_path} 
                                alt="Review" 
                    ...
                    {review.product && (
                      <p className="text-xs font-bold text-maroon mt-4 bg-maroon/5 inline-block px-2 py-1 rounded-md">
                        {review.product.name}
                      </p>
                    )}
