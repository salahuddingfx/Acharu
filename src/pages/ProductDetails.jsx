  ...
  return (
    <>
      <Helmet>
        ...
        <meta property="og:site_name" content={settings.store_name || 'Acharu'} />
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            "brand": {
              "@type": "Brand",
              "name": settings.store_name || "Acharu"
            },
            "offers": {
              "@type": "Offer",
          ...
          {product.category && (
            <>
              ...
              <Link 
                ...
                className="hover:text-maroon transition-colors truncate max-w-[100px] md:max-w-none"
              >
                ...
                <button 
                  ...
                  className={clsx(
                    ...
                    isOutOfStock 
                      ...
                      : "bg-maroon text-white hover:bg-maroon/90 hover:scale-[1.02] shadow-maroon/20"
                  )}
                >
                  <ShoppingBag size={18} />
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 md:grid-cols-2 gap-12">
                ...
                <div className="h-full">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    <button onClick={() => setRatingFilter(0)} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${ratingFilter === 0 ? 'bg-maroon text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>All</button>
                    {[5,4,3,2,1].map(star => (
                      <button key={star} onClick={() => setRatingFilter(star)} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${ratingFilter === star ? 'bg-maroon text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>{star}★</button>
                    ))}
                  </div>
          ...
          <div className="mt-10">
            <div className="flex justify-between items-end mb-10 px-4 md:px-0">
              ...
              <div className="hidden md:flex gap-4 ml-6">
                 <button 
                  onClick={() => scroll('left')}
                  className="w-12 h-12 rounded-full border border-slate-100 flex items-center justify-center text-slate-600 hover:bg-maroon hover:text-white transition-all shrink-0"
                ...
                >
                 ...
                 <button 
                  ...
                  className="w-12 h-12 rounded-full border border-maroon flex items-center justify-center text-maroon hover:bg-maroon hover:text-white transition-all shrink-0"
      ...
      <div className="lg:hidden fixed bottom-[68px] md:bottom-0 left-0 right-0 bg-white/95 backdrop-blur-sm border-t border-slate-100 z-[99] shadow-[0_-8px_24px_rgba(0,0,0,0.08)]">
        <div className="flex items-center gap-2 px-3 py-2.5 max-w-lg mx-auto">
          ...
          <div className="min-w-0 shrink-0">
            ...
            <p className="text-base font-black text-emerald-600 leading-none">৳{((selectedVariation ? selectedVariation.price : product.price) * quantity).toFixed(0)}</p>
          </div>
          ...
          <div className="flex items-center bg-slate-100 rounded-lg border border-slate-200 shrink-0">
            <button 
              onClick={() => updateProductQuantity(quantity - 1)}
              className="w-8 h-8 flex items-center justify-center text-slate-600 active:text-maroon transition-colors"
