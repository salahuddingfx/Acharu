              ...
              <div className="mb-8 p-5 bg-slate-50 rounded-2xl border border-slate-100">
                ...
                <div className="flex gap-2">
                  <input 
                    ...
                    className="flex-grow min-w-0 bg-white border border-slate-200 rounded-xl px-4 py-2.5 font-bold text-slate-800 text-xs focus:ring-2 focus:ring-maroon/20 transition-all"
                  />
                  <button 
                    onClick={handleApplyCoupon}
                    disabled={couponLoading || !couponCode}
                    className="shrink-0 bg-slate-900 text-white px-4 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-maroon transition-all disabled:opacity-50"
                  >
                    {couponLoading ? '...' : 'Apply'}
                  </button>
                </div>
                {appliedCoupon && (
                  <div className="mt-3 flex items-center justify-between text-emerald-600 bg-emerald-50 p-2.5 rounded-xl border border-emerald-100">
                    ...
                    <div className="flex-grow">
                      <div className="flex justify-between items-start">
                        <p className="font-bold text-slate-800 text-sm line-clamp-1">
                          {item.name} {item.variation_info && <span className="text-maroon">({item.variation_info})</span>}
                        </p>
                        <button 
                          onClick={() => {
